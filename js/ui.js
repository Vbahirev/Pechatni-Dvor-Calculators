...
// === ВЫБОР МАТЕРИАЛОВ (Показываем группы по-русски) ===
export function renderMaterialsSelect() {
    const s = document.getElementById('newPartMaterial'); 
    s.innerHTML = '';
    const groups = {}; 
    const groupNames = { 'plywood': 'ДЕРЕВО', 'acrylic': 'ПЛАСТИК', 'metal': 'МЕТАЛЛ', 'other': 'ДРУГОЕ' };
...
// === ГЕНЕРАЦИЯ МЕНЮ "БАЗА ДАННЫХ" (РУССКИЕ ФОРМЫ) ===
export function renderSettingsLists() {
    
    // Формы добавления: Отправляем value="Дерево" (Русский текст)
    ...
    document.getElementById('addRateForm').innerHTML = renderAddForm('addRate', `
        <option value="метр">За метр (рез)</option>
        <option value="м2">За м² (площадь)</option>
    `);
    document.getElementById('addExtraForm').innerHTML = renderAddForm('addExtra', `
        <option value="шт">За штуку</option>
        <option value="фикс">Фикс. цена</option>
    `);
