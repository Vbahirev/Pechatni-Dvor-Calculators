...
export function renderMaterialsSelect() {
    ...
    // Группируем материалы для красивого списка
    const groups = {}; 
    
    // Переводим технические группы на русский для отображения
    const groupNames = {
    ...
    state.materialList.forEach(m => {
        ...
        // Если такой группы нет в словаре, показываем как есть, но большими буквами
...
export function renderProcessingSection(){const ar=state.processingRates.filter(r=>r.type==='area');const c=document.getElementById('processingContainer');if(ar.length===0){c.innerHTML=`<div class="text-xs text-slate-400 text-center italic py-2">Нет услуг</div>`;return;}c.innerHTML=ar.map(r=>`<div id="box_${r.id}" class="border border-slate-200 rounded-lg p-3 transition-colors bg-white ${state.activeProcessingChecks[r.id]?'border-orange-200 bg-orange-50/30':''}"><div class="flex items-center justify-between mb-2"><div class="flex items-center gap-2"><input type="checkbox" id="has_${r.id}" onchange="toggleProcessing('${r.id}')" class="w-4 h-4 text-orange-600 rounded focus:ring-orange-500" ${state.activeProcessingChecks[r.id]?'checked':''}><label for="has_${r.id}" class="text-xs font-bold uppercase select-none cursor-pointer text-slate-700">${r.name}</label></div></div><div class="flex gap-2 items-center ${state.activeProcessingChecks[r.id]?'':'opacity-50 pointer-events-none'}" id="inputs_${r.id}"><input type="number" id="w_${r.id}" placeholder="Ш" class="input-field h-8 w-14 text-xs bg-slate-50" oninput="calcArea('${r.id}')"><span class="text-slate-300 text-xs">x</span><input type="number" id="h_${r.id}" placeholder="В" class="input-field h-8 w-14 text-xs bg-slate-50" oninput="calcArea('${r.id}')"><i class="fas fa-arrow-right text-slate-300 text-[10px] mx-1"></i><input type="number" id="area_${r.id}" value="${state.activeProcessingAreas[r.id]||0}" class="input-field h-8 flex-grow text-xs font-bold text-orange-600 bg-white" oninput="manualArea('${r.id}',this.value)"><span class="text-[10px] text-slate-400">см²</span></div></div>`).join('');}
...
// === РЕНДЕР НАСТРОЕК (РУССКИЕ СПИСКИ) ===
export function renderSettingsLists() {
    
    // Функция создания формы добавления с РУССКИМИ выпадающими списками
    const renderAddForm = (id, optionsHtml) => `
        <div class="flex gap-2 items-center mb-4 bg-slate-50 p-2 rounded border border-slate-100">
            <input type="text" id="${id}Name" placeholder="Название (напр. Ольха)" class="input-field h-8 flex-grow text-xs text-left px-2 bg-white">
            
            <select id="${id}Param" class="input-field h-8 w-32 text-xs px-1 bg-white cursor-pointer font-bold text-slate-600">
                ${optionsHtml}
            </select>
            
            <input type="number" id="${id}Cost" placeholder="Цена" class="input-field h-8 w-20 text-xs text-right px-2 bg-white">
    ...
    // 1. Форма МАТЕРИАЛОВ (Выбор группы: Дерево или Пластик)
    // value="plywood" - это то, что уйдет в базу (английский код)
    // "Дерево / Фанера" - это то, что вы увидите на экране
    ...
    // 2. Форма ТАРИФОВ (Выбор типа: Метр или Площадь)
    document.getElementById('addRateForm').innerHTML = renderAddForm('addRate', `
        <option value="linear">За погонный метр (рез)</option>
        <option value="area">За кв. метр (площадь)</option>
    `);

    // 3. Форма ДОП УСЛУГ (Выбор типа: Штука или Фикс)
    document.getElementById('addExtraForm').innerHTML = renderAddForm('addExtra', `
        <option value="qty">За штуку</option>
        <option value="fixed">Фиксированная цена</option>
    `);

    // Отрисовка самих списков (существующие позиции)
    const makeRow = (i, unit) => `
        <div id="row_${i.id}" class="flex justify-between items-center p-2 border-b border-slate-50 text-xs hover:bg-slate-50 transition">
            ...
            <div class="flex items-center gap-2 justify-end flex-grow">
                <input type="number" value="${i.cost}" id="cost_${i.id}" class="w-16 border border-slate-300 rounded h-7 text-sm text-right focus:border-orange-500 outline-none px-1">
